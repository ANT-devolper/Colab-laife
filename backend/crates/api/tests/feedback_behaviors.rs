//! Integration test for the RBAC-guarded `/feedback-behaviors` CRUD routes. A
//! feedback behavior (a scored DISC-values line) lives in the tenant schema and
//! references a feedback. Provisions a tenant, drives the routes over real HTTP,
//! and covers the authorized admin (including referenced-feedback validation) and
//! a member that lacks the permission.
//!
...
#[tokio::test]
async fn admin_can_create_list_update_and_deactivate_feedback_behaviors() {
    ...
    // Create.
    let created = server
        .post("/feedback-behaviors")
        .add_header("Authorization", &auth)
        .json(&json!({
            "feedback_id": feedback_id,
            "value_description": "Ownership",
            "behavior_description": "Takes initiative on incidents",
            "score": 4
        }))
        .await;
    ...
    assert_eq!(body["value_description"], "Ownership");
    assert_eq!(body["score"], 4);
    assert_eq!(body["active"], true);

    // List filtered by feedback shows it.
    let listed = server
        .get(&format!("/feedback-behaviors?feedback_id={feedback_id}"))
        .add_header("Authorization", &auth)
        .await;
    ...
    // Update the score and an optional note.
    let updated = server
        .patch(&format!("/feedback-behaviors/{id}"))
        .add_header("Authorization", &auth)
        .json(&json!({ "score": 5, "behavior_obs": "Consistently" }))
        .await;
    updated.assert_status(StatusCode::OK);
    let updated_body = updated.json::<serde_json::Value>();
    assert_eq!(updated_body["score"], 5);
    assert_eq!(updated_body["behavior_obs"], "Consistently");
    // Untouched field preserved.
    assert_eq!(updated_body["value_description"], "Ownership");

    // Deactivate (soft delete).
    server
        .delete(&format!("/feedback-behaviors/{id}"))
        .add_header("Authorization", &auth)
        .await
    ...
    assert_eq!(
        ...
        0,
        "a deactivated behavior should not be listed"
    );
}
...
async fn creating_for_an_unknown_feedback_is_rejected() {
    ...
    server
        ...
        .json(&json!({
            ...
            "value_description": "Ownership",
            "behavior_description": "Takes initiative",
            "score": 3
        }))
        .await
