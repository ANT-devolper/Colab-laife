//! Integration test for the RBAC-guarded `/roles` CRUD routes. Roles live in the
//! tenant schema and carry the full legacy set of description fields. Provisions
//! a tenant and drives the routes over real HTTP, covering the authorized admin
//! and a member that lacks the permission.
//!
...
#[tokio::test]
async fn admin_can_create_list_update_and_deactivate_roles() {
    let (server, _db, _provisioned) = setup().await;
    let token = token(&server, "admin@acme.test", ADMIN_PASSWORD).await;
    let auth = format!("Bearer {token}");

    // Create, carrying a couple of the optional description fields.
    let created = server
        .post("/roles")
        .add_header("Authorization", &auth)
        .json(&json!({
            "name": "Backend Engineer",
            "objective": "Build and run the API",
            "requirement_knowledge": "Rust, PostgreSQL"
        }))
        .await;
    created.assert_status(StatusCode::CREATED);
    let body = created.json::<serde_json::Value>();
    let id = body["id"].as_str().expect("id").to_owned();
    assert_eq!(body["name"], "Backend Engineer");
    assert_eq!(body["objective"], "Build and run the API");
    assert_eq!(body["requirement_knowledge"], "Rust, PostgreSQL");
    ...
    let roles = listed.json::<serde_json::Value>();
    assert!(
        roles
            .as_array()
            .expect("array")
            .iter()
            .any(|r| r["id"] == id && r["name"] == "Backend Engineer"),
        "the created role should be listed"
    );

    // Update the name and one description field.
    let updated = server
        .patch(&format!("/roles/{id}"))
        .add_header("Authorization", &auth)
        .json(&json!({ "name": "Platform Engineer", "objective": "Own the platform" }))
        .await;
    updated.assert_status(StatusCode::OK);
    let updated_body = updated.json::<serde_json::Value>();
    assert_eq!(updated_body["name"], "Platform Engineer");
    assert_eq!(updated_body["objective"], "Own the platform");
    // Untouched field is preserved.
    assert_eq!(updated_body["requirement_knowledge"], "Rust, PostgreSQL");

    // Deactivate (soft delete).
    server
        .delete(&format!("/roles/{id}"))
        .add_header("Authorization", &auth)
        .await
    ...
    assert!(
        !after
            ...
            .iter()
            .any(|r| r["id"] == id),
        "a deactivated role should not be listed"
    );
}
