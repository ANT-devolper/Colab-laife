//! Integration test for the RBAC-guarded `/feedbacks` CRUD routes. Feedback lives
//! in the tenant schema and references a collaborator. Provisions a tenant, drives
//! the routes over real HTTP, and covers the authorized admin (including
//! referenced-collaborator validation) and a member that lacks the permission.
//!
...
#[tokio::test]
async fn admin_can_create_list_update_and_deactivate_feedback() {
    ...
    let created = server
        ...
        .json(&json!({
            ...
            "expectation_contract_observation": "Keep up the pace",
            "status": "done"
        }))
        .await;
    ...
    let listed = server
        .get(&format!("/feedbacks?collaborator_id={collaborator_id}"))
        .add_header("Authorization", &auth)
        .await;
    listed.assert_status(StatusCode::OK);
    let feedbacks = listed.json::<serde_json::Value>();
    assert!(
        feedbacks
            .as_array()
            .expect("array")
            .iter()
            .any(|f| f["id"] == id),
        "the created feedback should be listed"
    );

    // Update the observation and status.
    let updated = server
        .patch(&format!("/feedbacks/{id}"))
        .add_header("Authorization", &auth)
        .json(
            &json!({ "status": "scheduled", "expectation_contract_observation": "Revisit goals" }),
        )
        .await;
    updated.assert_status(StatusCode::OK);
    let updated_body = updated.json::<serde_json::Value>();
    assert_eq!(updated_body["status"], "scheduled");
    assert_eq!(
        updated_body["expectation_contract_observation"],
        "Revisit goals"
    );
    // Untouched field preserved.
    assert_eq!(updated_body["collaborator_id"], collaborator_id);

    // Deactivate (soft delete).
    server
        .delete(&format!("/feedbacks/{id}"))
        .add_header("Authorization", &auth)
        .await
    ...
    assert!(
        !after
            ...
            .iter()
            .any(|f| f["id"] == id),
        "a deactivated feedback should not be listed"
    );
}
