//! Integration test for the RBAC-guarded `/annotations` CRUD routes. An annotation
//! (a quick scored note) lives in the tenant schema and references a collaborator.
//! Provisions a tenant, drives the routes over real HTTP, and covers the
//! authorized admin (including referenced-collaborator validation) and a member
...
/// Creates a collaborator via the API and returns its id.
async fn create_collaborator(server: &TestServer, auth: &str) -> String {
    let response = server
        .post("/collaborators")
...
#[tokio::test]
async fn admin_can_create_list_update_and_deactivate_annotations() {
    let (server, _db, _provisioned) = setup().await;
    let token = token(&server, "admin@acme.test", ADMIN_PASSWORD).await;
    let auth = format!("Bearer {token}");

    let collaborator_id = create_collaborator(&server, &auth).await;
    ...
    let created = server
        ...
        .json(&json!({
            ...
            "note_date": "2026-06-01T10:00:00Z",
            "score1_number": 8,
            "score1_type": "performance",
            "main_note": "Strong delivery this sprint"
        }))
        .await;
    ...
    assert_eq!(body["score1_number"], 8);
    assert_eq!(body["score1_type"], "performance");
    assert_eq!(body["ask_amount_days"], false);
    assert_eq!(body["active"], true);

    // List filtered by collaborator shows it.
    let listed = server
        .get(&format!("/annotations?collaborator_id={collaborator_id}"))
        .add_header("Authorization", &auth)
        .await;
    listed.assert_status(StatusCode::OK);
    assert_eq!(
        listed.json::<serde_json::Value>().as_array().unwrap().len(),
        1
    );

    // Update the main note and a score.
    let updated = server
        .patch(&format!("/annotations/{id}"))
        .add_header("Authorization", &auth)
        .json(&json!({ "score1_number": 9, "observation": "Keep mentoring" }))
        .await;
    updated.assert_status(StatusCode::OK);
    let updated_body = updated.json::<serde_json::Value>();
    assert_eq!(updated_body["score1_number"], 9);
    assert_eq!(updated_body["observation"], "Keep mentoring");
    // Untouched field preserved.
    assert_eq!(updated_body["score1_type"], "performance");

    // Deactivate (soft delete).
    server
        .delete(&format!("/annotations/{id}"))
        .add_header("Authorization", &auth)
        .await
    ...
    assert_eq!(
        ...
        0,
        "a deactivated annotation should not be listed"
    );
}

#[tokio::test]
async fn creating_for_an_unknown_collaborator_is_rejected() {
    let (server, _db, _provisioned) = setup().await;
    let token = token(&server, "admin@acme.test", ADMIN_PASSWORD).await;
    let auth = format!("Bearer {token}");
